...
public class StackListImplementation<T> implements Stack<T> {
	...
	public T peek() {
		if (isEmpty())
			return null;
		if(logFlag)System.out.println("[Peeking Data]: " + topHead.getData());
	...
	public T pop() {
		...
		if(logFlag)System.out.println("[Popping data]: " + data);
		return data;
	}

	@Override
	public void push(T data) {
		if(logFlag)System.out.println("[Pushing data]: " + data);
		ListNode<T> temp = new ListNode<T>(data, topHead);
		topHead = temp;
	}

	public static void main(String[] args) {
		StackListImplementation<String> stack = new StackListImplementation<>(true);
		stack.push("1");
		stack.push("2");
		stack.push("3");
		stack.push("4");
