 ...
 * interface class, and it in turn "delegates" all requests to the
 ...
 * 
 * you want run-time binding of the implementation, you have a proliferation of
 * classes resulting from a coupled interface and numerous implementations, you
 * want to share an implementation among multiple objects, you need to map
 * orthogonal class hierarchies. Consequences include:
 * 
 * decoupling the object's interface, improved extensibility (you can extend
 * (i.e. subclass) the abstraction and implementation hierarchies
 * independently), hiding details from clients. Bridge is a synonym for the
 * "handle/body" idiom. This is a design mechanism that encapsulates an
 * implementation class inside of an interface class. The former is the body,
 * and the latter is the handle. The handle is viewed by the user as the actual
 * class, but the work is done in the body.
